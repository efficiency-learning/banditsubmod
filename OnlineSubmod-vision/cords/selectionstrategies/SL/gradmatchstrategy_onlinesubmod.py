...
class GradMatchStrategy(DataSelectionStrategy):
    ...
    def select(self, budget, model_params):
        ...
        print("Select", self.selection_type )
        ...
        if self.selection_type == 'PerClass':
            ...
            # print("budget", budget, self.trainloader.batch_size ,math.ceil(budget / self.trainloader.batch_size) )
            # print("batch", len(batch_wise_indices), len(batch_wise_indices[0]), batch_wise_indices[0][0:16])
            print("idxs", len(idxs_temp), idxs_temp[0:15])
            print("gam", len(gammas_temp), gammas_temp[0:15])
            print("Grads", trn_gradients.shape, self.val_grads_per_elem.shape, sum_val_grad.shape )
            print("****")
