...
args = dict(
    select_every=dict(count=10, mode="epoch"),
    warm_start = 20,
    # warm_every = 21,
    warm_end = 20,
    ...
    def train(self, **kwargs):
        ...
        for epoch in range(start_epoch, num_epochs+1):
            ...
            for batch_idx, (features, targets, images) in enumerate(tqdm(dataloader, desc=f"Epoch {epoch+1}/{num_epochs}")):
                ...
                if(self.do_warm_end(args, epoch, allow_zero=False)):
                ...
                elif(args["warm_start"] <= epoch+1 and self.do_dss(args, step, epoch, allow_zero=True)):
                    len_opt = 0
                    opt_grads = None
                    ...
                    '''
                    funcs = [
                        submodlib.GraphCutMutualInformationFunction(n = n, data = feature_mat, queryData=val_features, metric="euclidean", num_queries=num_queries),
                        submodlib.LogDeterminantMutualInformationFunction(n, lambdaVal = 0.1,  data = feature_mat,queryData=val_features, metric="euclidean",num_queries=num_queries),
                        submodlib.FacilityLocationMutualInformationFunction(n, data = feature_mat,queryData=val_features, metric="euclidean",num_queries=num_queries),
                        submodlib.ConcaveOverModularFunction(n, data = feature_mat,queryData=val_features, metric="euclidean",num_queries=num_queries),
                    ...
                    mode, greedyFinal, _best_arm = submod.eps_greedy_composition_batched(model, val_samples, 
                                                                criterion, global_step, funcs, submod_budget, moment_sum, args, 
                                                                bak_trainloader, greedyOnly=False, 
                    ...
                    # print("Batch list", batch_wise_indices.shape)
                    batch_wise_indices = batch_wise_indices.tolist()
                    # print('gammaa', gammas)
                    
                    # for i in range(num_batches):
                    print("selected batches",mode, best_arm,"::", idxs)
                    for i in range(len(idxs)):
                        tmp = batch_wise_indices[idxs[i]]
                        idxs.extend(tmp)
                        gammas.extend(list(gammas[i] * np.ones(len(tmp))))
                    idxs, gammas = submod.get_new_idxs(idxs, gammas, batch_size,  budget_num_batches, trainloader)
                    # epoch = epoch-1
                    subset_loader = self.refresh_subset_loader(train_features_dataset, 
                                                            [i for i in idxs], [i for i in gammas])
