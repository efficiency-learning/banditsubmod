...
def compute_dino_image_embeddings(images, device, return_tensor=False, model=None, feature_extractor=None):
    # model = ViTModel.from_pretrained('facebook/dino-vitb16')
    # model= model.to(device)
...
def load_dataset(dataset_name, data_dir, seed, return_valid=False, return_test=False):
    if dataset_name == "glue_sst2":
        ...
        fullset = torchvision.datasets.CIFAR10(root=data_dir, split='train', download=True, transform=None)
        testset = torchvision.datasets.CIFAR10(root=data_dir, split='val', download=True, transform=None)
        validation_set_fraction = 0.1
        num_fulltrn = len(fullset)
        num_val = int(num_fulltrn * validation_set_fraction)
        num_trn = num_fulltrn - num_val
